...
st.markdown("""
    ...
    .stNumberInput label {
        ...
        color: #4169E1 !important; /* 青系に変更して区別 */
        font-weight: 800 !important;
    }
    .main-input div[data-baseweb="input"] {
...
with st.expander("⚙️ 基本情報設定 (990-FD0基準)", expanded=False):
    ...
    with col2:
        s_max = st.number_input("スケール上限 (100%)", value=100.00) # 仮に100
...
if mode == "伝送値(bit)":
    # 伝送値を整数で入力（初期値を下限の990に設定）
    val = st.number_input("現在の伝送値を入力", value=990, step=1, format="%d")
...
# 伝送値の表示（整数に丸める）
display_bit = int(res_bit)

# --- 4. 表示エリア ---
st.subheader("📊 換算結果")
st.metric("伝送値", f"{display_bit} bit")
st.metric("スケール換算値", f"{res_scale:,.3f}")
st.metric("電流値", f"{res_ma:,.2f} mA")
st.metric("電圧値", f"{res_v:,.3f} V")

st.markdown("---")
st.caption(f"現在の計算ベース: {percent*100:.2f} % (伝送範囲: 990 - 4048)")
